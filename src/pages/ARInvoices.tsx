...
const STATUS_STYLES: Record<string, string> = {
  Open: 'badge-info', Partial: 'badge-warning', Paid: 'badge-success', Overdue: 'badge-danger', Draft: 'badge-gray', Void: 'badge-gray',
};
...
export default function ARInvoices() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ar/invoices?${params}`).then((res) => res.json()).then(setInvoices).finally(() => setLoading(false));
  }, [filters.status]);
          ...
          <div className="flex flex-wrap gap-4">
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                <input type="text" placeholder="Search invoices..." value={filters.search}
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead>
                <tr>
                  <th>Invoice #</th><th>Customer</th><th>Description</th><th>Invoice Date</th>
                  <th>Due Date</th><th className="text-right">Total</th><th className="text-right">Balance</th><th>Status</th>
                </tr>
              </thead>
              <tbody>
                {filteredInvoices.map((invoice) => (
                  <tr key={invoice.id} className="cursor-pointer hover:bg-gray-50">
                    <td className="font-mono text-sm font-medium text-brand-600">{invoice.invoice_number}</td>
                    <td><div><p className="font-medium">{invoice.customer_name}</p><p className="text-xs text-gray-500">{invoice.customer_number}</p></div></td>
                    ...
                    <td className="text-right font-mono font-medium">{formatCurrency(invoice.balance_due)}</td>
                    <td><span className={`badge ${STATUS_STYLES[invoice.current_status] || 'badge-gray'}`}>{invoice.current_status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
